...
function drawSpeedLines() {
  ...
  if (t <= 0) return;
  ctx.save();
  // Clip to canvas to prevent edge artifacts
  ctx.beginPath(); ctx.rect(1, 20, W - 2, H - 40); ctx.clip();
  ctx.lineCap = 'butt';
  const step = (H - 45) / 11;
  for (let i = 0; i < 12; i++) {
    ...
    const len = 7 + (i % 4) * 4;
    const x1 = x2 + len;
    if (x2 > W || x1 < 0) continue;
    ctx.globalAlpha = t * (0.04 + (i % 3) * 0.025);
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 0.28 + (i % 2) * 0.18;
    ctx.beginPath();
    ctx.moveTo(Math.min(x1, W - 1), baseY);
    ctx.lineTo(Math.max(x2, 1), baseY);
    ctx.stroke();
  }
  ctx.restore();
}
